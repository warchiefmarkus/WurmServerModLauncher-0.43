...
/*  424 */         str = "BOOTLOADER";
/*      */       } else {
/*  426 */         str = LogFactory.objectId(classLoader);
/*      */       } 
...
/*  443 */       LogFactory.logRawDiagnostic(this.diagnosticPrefix + msg);
/*      */     }
/*      */   }
/*      */ 
/*      */ 
...
/*  568 */       LogFactory.handleThrowable(t);
/*      */ 
/*      */       
...
/*      */       
/*      */       try {
/*  907 */         specifiedClass = getSystemProperty("org.apache.commons.logging.Log", null);
...
/*      */       
/*      */       try {
/*  922 */         specifiedClass = getSystemProperty("org.apache.commons.logging.log", null);
...
/*      */     
/*      */     while (true) {
/*  974 */       logDiagnostic("Trying to load '" + logAdapterClassName + "' from classloader " + LogFactory.objectId(currentCL)); try {
/*      */         Class clazz;
...
/* 1004 */           logDiagnostic("The log adapter '" + logAdapterClassName + "' is not available via classloader " + LogFactory.objectId(currentCL) + ": " + msg.trim());
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/* 1055 */         logDiagnostic("The log adapter '" + logAdapterClassName + "' is missing dependencies when loaded via classloader " + LogFactory.objectId(currentCL) + ": " + msg.trim());
/*      */ 
/*      */         
/*      */         break;
...
/* 1067 */         logDiagnostic("The log adapter '" + logAdapterClassName + "' is unable to initialize itself when loaded via classloader " + LogFactory.objectId(currentCL) + ": " + msg.trim());
/*      */ 
/*      */         
/*      */         break;
...
/* 1076 */         LogFactory.handleThrowable(t);
/*      */ 
/*      */ 
/*      */         
...
/* 1102 */         LogFactory.handleThrowable(t);
/* 1103 */         this.logMethod = null;
/* 1104 */         logDiagnostic("[INFO] '" + logAdapterClassName + "' from classloader " + LogFactory.objectId(currentCL) + " does not declare optional method " + "setLogFactory(LogFactory)");
/*      */       } 
/*      */ 
/*      */       
/* 1108 */       logDiagnostic("Log adapter '" + logAdapterClassName + "' from classloader " + LogFactory.objectId(logAdapterClass.getClassLoader()) + " has been selected for use.");
/*      */     } 
/*      */ 
/*      */     
...
/* 1339 */           logDiagnostic("Class '" + badClass.getName() + "' was found in classloader " + LogFactory.objectId(badClassLoader) + ". It is bound to a Log interface which is not" + " the one loaded from classloader " + LogFactory.objectId(logInterfaceClassLoader));
/*      */         
/*      */         }
/* 1342 */         catch (Throwable t) {
/* 1343 */           LogFactory.handleThrowable(t);
...
/* Location:              C:\Users\leo\Desktop\server.jar!\org\apache\commons\logging\impl\LogFactoryImpl.class
